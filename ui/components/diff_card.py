...
def render_diff_card(
    ...
    # Proof of Concept / Sandbox Verification Block
    poc_html = ""
    if poc and isinstance(poc, dict):
        poc_code = html.escape(poc.get("code", ""))
        poc_out = html.escape(poc.get("runtime_output", ""))
        poc_html = f"""
        <div style="margin-top: 8px; padding: 8px; background: rgba(15, 23, 42, 0.6); border: 1px dashed rgba(99, 102, 241, 0.4); border-radius: 6px;">
            ...
            <pre style="margin: 0; padding: 4px 8px; background: #0B0D13; border-radius: 4px; font-size: 0.76rem; color: #CBD5E1; overflow-x: auto;">{poc_code}</pre>
            <div style="margin-top: 4px; font-size: 0.76rem; font-family: monospace; color: #F87171; background: rgba(239, 68, 68, 0.1); padding: 2px 6px; border-radius: 4px;">
                {poc_out}
            </div>
        </div>
        """
    ...
    <div style="margin-top: 10px;">
        ...
        <div class="diff-fix-line">✅ + {safe_fix}</div>
    </div>
    """ if safe_fix else ""
    ...
    <div class="diff-card">
        ...
        <div class="diff-body">
            ...
            <div class="comment-thread">
                <div class="comment-author">🤖 PR Sage Verification Engine ({safe_ai})</div>
                <div class="comment-text"><b>Why:</b> {safe_desc}</div>
                {evidence_html}
                {impact_html}
                {poc_html}
                {fix_line_html}
